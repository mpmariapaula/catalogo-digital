...
export function Products() {
  const { products, deleteProduct, categories, subcategories, isLoading } = useData();
  ...
  const getSubcategoryName = (subcategoryId: string) => subcategories.find((s) => s.id === subcategoryId)?.name || '-';
                  ...
                  <TableRow key={product.id}>
                    ...
                    <TableCell>
                      <div className="max-w-xs">
                        ...
                        <p className="text-xs text-gray-500">{getSubcategoryName(product.subcategoryId)}</p>
                      </div>
                    </TableCell>
                    <TableCell>
                      <span className="text-xs font-mono bg-gray-100 px-2 py-1 rounded">{product.productId}</span>
                    </TableCell>
                    <TableCell className="text-sm">{getCategoryName(product.categoryId)}</TableCell>
                    <TableCell className="text-sm">R$ {product.price.toFixed(2).replace('.', ',')}</TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => handleDelete(product)} className="text-red-600 hover:text-red-700 hover:bg-red-50">
